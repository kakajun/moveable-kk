...
export default class LayerUtil {
  ...
  static findPathGroupLayer(layerIds) {
    ...
    layerIds.forEach((id) => {
      ...
      if (layer.type === 'group') groupLayerIdSet.add(id)
      let _pid = layer?.parent
      while (_pid) {
        const { parent, id, type } = layerConfigs[_pid]
        _pid = parent
        if (type === 'group') groupLayerIdSet.add(id)
      }
    })
  ...
  static findTopGroupLayer(layerIds, hasSelf = false) {
    ...
    layerIds.forEach((id) => {
      ...
      if (hasSelf || _id !== id) groupLayerIdSet.add(_id)
    })
    return Array.from(groupLayerIdSet)
  }
  ...
  static hasSameGroup(layerIds) {
    ...
    if (layerIds.some((id) => layerConfigs[id].type !== 'group' && !layerConfigs[id].parent))
      return false
    const groupLayerIds = new Set()
    layerIds
      .filter((id) => layerConfigs[id].type !== 'group')
      .forEach((id) => {
