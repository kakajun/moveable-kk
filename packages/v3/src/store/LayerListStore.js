...
export default defineStore('layerList', {
  ...
  actions: {
    ...
    selectedChange(id, event) {
      ...
      const { layerConfigs } = designerStore()
      const { type, config } = layerConfigs[id]
      ...
      if (event.ctrlKey) {
        if (targetIds.includes(id)) {
          ...
          if (targetIds.length === 0) {
            ...
            const firstLock = layerConfigs[targetIds[0]].config.lock
            if (lock !== firstLock) {
              if (groupLayer)
                selectedLayerIds = [...targetIds, ...LayerUtil.findAllChildLayer([id])].filter(
                  (id) => !layerConfigs[id].config.lock
                )
              else
                selectedLayerIds = [...targetIds, id].filter((id) => !layerConfigs[id].config.lock)
            } else {
              if (groupLayer)
      ...
      let finalLock = layerConfigs[selectedLayerIds[0]]?.config.lock
